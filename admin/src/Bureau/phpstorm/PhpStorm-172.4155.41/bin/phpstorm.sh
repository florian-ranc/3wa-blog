...
if [ -z "$JDK" -a -s "$HOME/.PhpStorm2017.2/config/phpstorm.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.PhpStorm2017.2/config/phpstorm.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.PhpStorm2017.2/config/phpstorm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.PhpStorm2017.2/config/phpstorm$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,probe_disable=*,sessionname=PhpStorm2017.2"
  fi
fi
